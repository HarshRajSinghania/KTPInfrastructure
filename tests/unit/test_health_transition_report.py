...
def join_only(tmp_path, items, name):
    """join_keys alone, under the script's own flags. rc != 0 means the real run
    would have died before the Discord POST."""
    body = STRICT + block("ktp-alert-join") + "\n"
    body += "items=(%s)\n" % " ".join('"%s"' % i for i in items)
    body += 'n=$(join_keys ${items[@]+"${items[@]}"})\n'
    body += 'echo "JOINED=[${n}]"\necho REACHED_THE_POST\n'
    r = bash(tmp_path, body, name, expect_ok=False)
    return r.returncode, r.stdout


def test_one_item_carries_no_separator(tmp_path):
    rc, out = join_only(tmp_path, ["mysql.service=failed"], "one.sh")
    assert rc == 0
    assert "JOINED=[mysql.service=failed]" in out


def test_an_item_with_a_space_stays_one_item(tmp_path):
    rc, out = join_only(tmp_path, ["ktp-render-banlist.timer=inactive/enabled",
                                   "hltv-demo-renamer-read=atl bm"], "space.sh")
    assert rc == 0
    assert "JOINED=[ktp-render-banlist.timer=inactive/enabled,hltv-demo-renamer-read=atl bm]" in out


def test_printf_format_injection_is_not_possible(tmp_path):
    """`-n` and `%s` are real item shapes only by accident, but a helper that
    passed either to printf as a FORMAT would mangle or swallow the line."""
    rc, out = join_only(tmp_path, ["%s%s", "-n"], "fmt.sh")
    assert rc == 0
    assert "JOINED=[%s%s,-n]" in out


def test_the_helper_cannot_fail_on_any_input(tmp_path):
    """Whatever it is handed the run must continue: this line is cosmetic and
    must never decide whether an alert is delivered.

    One deliberate behaviour change against the pre-fix spelling: an empty
    element now renders as a gap (`a,,b`) instead of being dropped. The dropping
    was a side effect of `grep -v '^$'` rather than the point, and an empty key
    reaching the down set is itself worth seeing in the log.
    """
    for i, items in enumerate([[], [""], ["", ""], ["a"], ["*"], ["$(echo hi)"],
                               ["-n"], ["%s"], ["a", "", "b"]]):
        rc, out = join_only(tmp_path, items, "any%d.sh" % i)
        assert rc == 0, items
...
def test_a_transitional_state_is_re_sampled(tmp_path, state):
    out = settle(tmp_path, [state, "active"], "trans_%s" % state)
    assert "STATE=active" in out
    assert "SLEPT=1" in out


@pytest.mark.parametrize("state", ["active", "inactive", "failed", "unknown", ""])
def test_a_non_transitional_state_is_taken_at_its_word(tmp_path, state):
    """The control on the test above. A `case` that matched everything would pass
    there and suppress the whole check; it fails here, because round two says
    `active` for each of these and the first reading must survive it.

    `inactive` and `failed` are the ones that matter: the 2026-08-25 renamer
    wedge went unreported for two days, and a grace period over a terminal state
    is the same class of mistake.
    """
    out = settle(tmp_path, [state, "active"], "term_%s" % (state or "empty"))
    assert "STATE=%s\n" % state in out
    assert "SLEPT=0" in out


def test_the_settle_delay_is_longer_than_restartsec():
    """hltv@.service is RestartSec=10. A shorter settle re-reads an auto-restart
    hold while it is still `activating`, and the transient comes straight back."""
    src = block("ktp-alert-settle")
    assert int(src.split("SETTLE_SECONDS:-")[1].split("}")[0]) > 10


def test_one_sleep_for_the_whole_run_not_one_per_unit(tmp_path):
    """Twelve transitional proxies must still cost a single delay. At the shipped
    20s default, per-unit would be eight minutes of a cron job."""
    fake = tmp_path / "manybin"
    fake.mkdir()
    rnd = tmp_path / "round"
    rnd.write_text("1\n", encoding="utf-8", newline="\n")
    (fake / "systemctl").write_text(
        "#!/bin/bash\n"
        "r=$(cat %s)\n"
        'if [ "$r" = "1" ]; then echo deactivating; else echo active; fi\n' % rnd.as_posix(),
        encoding="utf-8", newline="\n")
    (fake / "systemctl").chmod(0o755)
    body = ("export PATH=%s:$PATH\n" % fake.as_posix()) + STRICT
    body += "SETTLE_SECONDS=0\n" + block("ktp-alert-settle") + "\n"
    body += "SLEEPS=0\n"
    body += "sleep() { SLEEPS=$((SLEEPS + 1)); echo 2 > %s; }\n" % rnd.as_posix()
    body += "for u in $(seq 27020 27031); do settled_state hltv@$u; echo \"$u=$SETTLED\"; done\n"
    body += 'echo "SLEEPS=$SLEEPS"\n'
    out = bash(tmp_path, body, "many.sh").stdout
    assert "SLEEPS=1" in out
    assert out.count("=active") == 12
