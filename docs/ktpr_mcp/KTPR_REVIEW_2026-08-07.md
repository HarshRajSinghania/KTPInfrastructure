...
> ✅ **RESOLVED 2026-08-30 — `afraznein/KTPInfrastructure`#196 (`d5018f6`).** The code now takes the
> docstring's side: every per-half rate divides by the HLStatsX half count, and `_hud_half_count`
> is informational only — `hud_halves` is carried on `RawMatchStats` and is never a denominator.
> The worked-example figures in `KTPR_CALCULATION.md` were regenerated against the post-#196 data
> layer on 2026-09-13, and that doc now says how to regenerate them (`ktpr_examples.py`). Under the
> old weights bR0M and hildebrand now come out level, so the reversal it narrates is owed to the
> 2026-08-08 changes alone. The text below is the original finding, kept as written.
