...
echo "  If uname shows a GENERIC kernel, GRUB entry 0 is not the lowlatency kernel."
echo "    audit (read-only):  $SCRIPT_DIR/../scripts/fix-grub-default-kernel.sh"
echo "    fix: GRUB_DEFAULT=0 in /etc/default/grub, plus /etc/default/grub.d/99-ktp-kernel-flavour.cfg"
echo "         containing GRUB_FLAVOUR_ORDER=\"lowlatency\"; update-grub && reboot"
echo "         (the audit's --fix writes the drop-in). Never pin a kernel title with grub-set-default."
echo "**********************************************************************"
echo
echo "Status:"
